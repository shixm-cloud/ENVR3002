...
\\[
z_{k+1} - z_k = \frac{R T_{k}}{g} \ln\left(\frac{p_k}{p_{k+1}}\right)
\\]

where \\(T_k\\) is the mean temperature of the atmosphere layer between height \\(z_{k}\\) and \\(z_{k+1}\\) (\\(p_{k+1}\\) and \\(p_k\\)). Because it uses temperature directly, instead of calculating density as intermediate step, it is more convinient.
